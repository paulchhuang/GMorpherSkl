...
__global__ void CUDA_PredEStep_Prod(const int     numTargets,
...
__global__ void CUDA_PredEStep_GMean(	const int     numTargets,
										const int     numPatches,
										const float   sigma,
										const float   EOutlier,
										const float   dotThresh,
										...
										int*    targets )
{
	...
	w_line[numPatches] = __powf(w_line[numPatches], 1.0f/num_nonZero);	/*geometric mean*/
	t_line[numPatches]    = -1;
	sumWeights += w_line[numPatches];
...
__global__ void CUDA_PredEStep( const int     numTargets,
                                const int     numPatches,
                                const float   sigma,
                                const float   EOutlier,
                                const float   dotThresh,
								...
								double*	bpProb_device)
{
	...
	float*  w_line = weights + (numPatches+1)*vit;
	int*    t_line = targets + (numPatches+1)*vit;
	double* p_line = bpProb_device+numPatches*vit;
	...
	for( int pi=0; pi<numPatches; ++pi)
	{
		...
		if( minVal == FLT_MAX ) w_line[pi] = 0;
			...
			float wi = expf(-minVal*half_sigmainv2)*p_line[pi];
			sumWeights+= wi;
			w_line[pi] = wi;
		}
		t_line[pi] = minIdx - beg_ptr; // since we have the indices multiplied by 2
                              ...
                              void*       targets_device)
{
	...
	int numBlocks   = (numTargets / CUDA_ICPPRED_ESTEP_TPB);
	if( numTargets % CUDA_ICPPRED_ESTEP_TPB ) numBlocks++;
	...
	CUDA_PredEStep<<<numBlocks, CUDA_ICPPRED_ESTEP_TPB>>>( numTargets,
	                                                       numPatches,
	                                                       sigma,
	                                                       EOutlier,
	                                                       dotThresh,
	...
	/*CUDA_PredEStep_GMean<<<numBlocks, CUDA_ICPPRED_ESTEP_TPB>>>( numTargets,
																numPatches,
																sigma,
																EOutlier,
																dotThresh,
	...
	/*CUDA_PredEStep_Prod<<<numBlocks, CUDA_ICPPRED_ESTEP_TPB>>>( numTargets,
																numPatches,
																sigma,
																EOutlier,
																dotThresh,
