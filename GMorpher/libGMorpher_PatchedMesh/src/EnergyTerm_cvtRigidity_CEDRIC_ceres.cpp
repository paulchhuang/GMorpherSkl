 ...
 * *************************************************/
#include <EnergyTerm_cvtRigidity_CEDRIC_ceres.h>
#include <GraphUtils.h>

namespace CVTClusteredCloud
{
	
EnergyTerm_cvtRigidity_CEDRIC_ceres::
EnergyTerm_cvtRigidity_CEDRIC_ceres(const CVTClusteredCloud&        PC) :
m_sadj         (PC.sadj()),
...
EnergyTerm_cvtRigidity_CEDRIC_ceres::~EnergyTerm_cvtRigidity_CEDRIC_ceres()
{
	delete[] m_dX0_smooth;	
	delete[] m_w_smooth;
}

void EnergyTerm_cvtRigidity_CEDRIC_ceres::addResidualBlock(ceres::Problem& problem, double* const RT, double* const JX)
{
	int numPatches = m_pv_bounds.size() - 1;	
	
//#pragma omp parallel for			// not able to parallelize the following loop? think again!
